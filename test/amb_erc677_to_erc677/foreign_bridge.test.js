...
contract('HomeAMBErc677ToErc677', async accounts => {
  ...
  describe('onTokenTransfer', () => {
    ...
    it('should emit UserRequestForAffirmation in AMB bridge', async () => {
      ...
      await homeBridge.onTokenTransfer(user, halfEther, '0x', { from: owner }).should.be.rejectedWith(ERROR_MSG)
      ...
      // When
      await erc677Token.transferAndCall(homeBridge.address, halfEther, '0x', { from: user }).should.be.fulfilled
      ...
      expect(await homeBridge.totalSpentPerDay(currentDay)).to.be.bignumber.equal(halfEther)
    })
    it('should be able to specify a different receiver', async () => {
      ...
      // When
      await erc677Token
        .transferAndCall(homeBridge.address, halfEther, '0x00', { from: user })
        .should.be.rejectedWith(ERROR_MSG)
      await erc677Token.transferAndCall(homeBridge.address, halfEther, user2, { from: user }).should.be.fulfilled
  ...
  describe('handleBridgedTokens', () => {
    ...
    beforeEach(async () => {
      ...
      await erc677Token.mint(homeBridge.address, twoEthers, { from: owner }).should.be.fulfilled
    ...
    it('should transfer locked tokens on message from amb', async () => {
      ...
      expect(initialEvents.length).to.be.equal(0)
      expect(await erc677Token.balanceOf(homeBridge.address)).to.be.bignumber.equal(twoEthers)
      ...
      expect(await erc677Token.balanceOf(homeBridge.address)).to.be.bignumber.equal(oneEther)
    ...
    it('should transfer locked tokens on message from amb with decimal shift of two', async () => {
      ...
      // Then
      expect(await homeBridge.totalExecutedPerDay(currentDay)).to.be.bignumber.equal(valueOnHome)
      expect(await erc677Token.balanceOf(homeBridge.address)).to.be.bignumber.equal(twoEthers.sub(valueOnForeign))
